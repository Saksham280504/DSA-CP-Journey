...
int MinInsertions(int ind1, int ind2, string s1, string s2, vector<vector<int>>& dp) {
    ...
    if(s1[ind1-1]==s2[ind2-1]) return 1 + MinInsertions(ind1-1, ind2-1, s1, s2,dp);
    return 0 + max(MinInsertions(ind1-1,ind2,s1,s2,dp), MinInsertions(ind1, ind2-1, s1, s2,dp));
}

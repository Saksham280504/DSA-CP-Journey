...
int maxProfit(int ind, int transaction, int n, vector<int>& prices,vector<vector<int>>& dp) {
    if(ind==n || transaction==4) return 0;
    if(dp[ind][transaction]!=-1) return dp[ind][transaction];
    if(transaction%2==0) return dp[ind][transaction] =  max(-prices[ind] + maxProfit(ind+1,transaction+1,n,prices,dp), 0 + maxProfit(ind+1,transaction,n,prices,dp));
    return dp[ind][transaction] = max(prices[ind] + maxProfit(ind+1,transaction+1,n,prices,dp), 0 + maxProfit(ind+1,transaction,n,prices,dp));
...
int main() {
    ...
    // This is the same question but we are using a different strategy for this.
    ...
    vector<vector<int>> dp1(n+1,vector<int>(5,-1));
    int profit1 = maxProfit(0,0,n,prices,dp1);
    cout << profit1 << endl;

    // Tabulation Approach:
    vector<vector<int>> dp2(n+1,vector<int>(5,0));

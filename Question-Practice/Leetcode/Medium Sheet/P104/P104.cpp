...
class Solution {
    ...
    bool checkValidString(string s) {
        ...
        for(auto it: s) {
            ...
            if(mini<0) mini = 0; // mini was taking the maximum chances of reducing itself, so if mini somehow reaches < 0, that means that when we took the path with maximum -1s(')'), that's when we came <0, we can prevent that condition by reducing one -1s to make mini again equal to 0
